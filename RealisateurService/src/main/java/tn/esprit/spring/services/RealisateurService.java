...
public class RealisateurService implements IRealisateurService{
   ...
   /* @Override
    public Realisateur affecterFilm(int id, Realisateur realisateur) {

            if(realisateurRepository.findById(id).isPresent()) {
                Realisateur existingRealisateur = realisateurRepository.findById(id).get();
                existingRealisateur.setFilm(realisateur.getFilm());
